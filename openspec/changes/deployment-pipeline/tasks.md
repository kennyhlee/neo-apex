...
- [ ] 2.7 Write `admindash/backend/Dockerfile` following the same pattern, exposing port 5610. (No special system dependencies needed; admindash-backend is a thin proxy with FastAPI + httpx only.)
...
- [ ] 6.4 Write `admindash/backend/fly.toml` with: `app = "admindash-backend"`, public `[http_service]` with `force_https = true`, `auto_stop_machines = true`, `auto_start_machines = true`, `min_machines_running = 0` (or 1 if cold-start latency is judged user-visible after first deploy), HTTP health check on `/api/health`, `[[vm]]` size `shared-cpu-1x` with 256mb memory
- [ ] 6.5 Create the four Fly.io apps via `flyctl apps create datacore|launchpad-api|papermite-api|admindash-backend` (idempotent if already created)
...
- [ ] 9.2 Generate four Fly.io deploy tokens via `flyctl tokens create deploy -a datacore|launchpad-api|papermite-api|admindash-backend`; store as `FLY_API_TOKEN_DATACORE`, `FLY_API_TOKEN_LAUNCHPAD`, `FLY_API_TOKEN_PAPERMITE`, `FLY_API_TOKEN_ADMINDASH` on the `production` environment
...
- [ ] 10.6 Add `deploy-admindash-backend` conditional on `module == 'admindash'`, running inside `environment: production`, with the same Docker-build-and-deploy pattern using `FLY_API_TOKEN_ADMINDASH` and the `admindash/backend/Dockerfile`
...
- [ ] 10.9 Add `deploy-admindash-frontend` conditional on `module == 'admindash'`. Note: an `admindash-v*` release tag fires both `deploy-admindash-backend` (Fly.io) and `deploy-admindash-frontend` (Cloudflare Pages) in the same workflow run
...
- [ ] 11.4 Create Pages project `admindash` with the same pattern, root `admindash/frontend`. Set `VITE_ADMINDASH_API_URL=https://api.admin.floatify.com` so the SPA targets the new admindash-backend Fly.io app.
...
- [ ] 12.3 Add the custom domain to each public Fly.io app via `flyctl certs add api.launchpad.floatify.com -a launchpad-api`, `flyctl certs add api.papermite.floatify.com -a papermite-api`, `flyctl certs add api.admin.floatify.com -a admindash-backend` and verify certificate issuance
...
- [ ] 15.2 Verify `datacore.internal:5800` is reachable from `launchpad-api` and `admindash-backend` by SSHing in via `flyctl ssh console -a launchpad-api` (and `-a admindash-backend`) and curling the endpoint
...
- [ ] 15.5 Cut an `admindash-v0.1.0` release; verify both `admindash-backend` (Fly.io) and `admindash` (Cloudflare Pages) deploy in the same workflow run, then walk through the admindash login + main flows from a browser to verify end-to-end
