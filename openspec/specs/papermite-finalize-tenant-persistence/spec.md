# papermite-finalize-tenant-persistence Specification

## Purpose

Defines the behavior of Papermite's `POST /api/tenants/{tenant_id}/finalize/commit` endpoint with respect to persisting extracted tenant entity values to DataCore alongside the model-definition write. Specifies the merge policy (existing user-typed values win over extraction), the base/custom split derivation, the skip conditions when extraction yields no usable tenant data, and the failure-isolation contract between the model write and the tenant write.
