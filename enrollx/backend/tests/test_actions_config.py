...
def draft_config(fake_dc, blocks=BLOCKS, version=1):
    return fake_dc.dc_create("acme", "registration_config", {
        "config_id": "cfg-d", "version": version,
...
def test_publish_with_business_config_id_404s(client, fake_dc):
    ...
    cfg = fake_dc.dc_create("acme", "registration_config", {
        "config_id": "ACME-RC260001", "version": 1,
        "status": "draft", "blocks": json.dumps(BLOCKS)})
...
def test_republishing_archived_config_preserves_its_version(client, fake_dc):
    ...
    v1 = fake_dc.dc_create("acme", "registration_config", {
        "config_id": "cfg-v1", "version": 1,
        "status": "archived", "blocks": json.dumps(BLOCKS)})
    v2 = fake_dc.dc_create("acme", "registration_config", {
        "config_id": "cfg-v2", "version": 2,
        "status": "published", "blocks": json.dumps(BLOCKS)})
...
def test_publish_leaves_exactly_one_published_config_for_the_tenant(client, fake_dc):
    """One lineage per tenant (spec §2): whatever else the tenant has, after a
    publish there is exactly one published config and the version continues
    the tenant's own lineage."""
    other = fake_dc.dc_create("acme", "registration_config", {
        "config_id": "other", "version": 9,
        "status": "published", "blocks": json.dumps(BLOCKS)})
    cfg = draft_config(fake_dc)
    assert publish(client, cfg["entity_id"]).status_code == 200
    ...
    assert published[0]["entity_id"] == cfg["entity_id"]
    assert int(published[0]["version"]) == 10
    assert fake_dc.get_entity("acme", "registration_config",
                              other["entity_id"])["status"] == "archived"
