...
/** Human-friendly read-only rendering of a value (arrays joined, else as text). */
function toReadonlyDisplay(v: unknown): string {
  if (Array.isArray(v)) return v.map((x) => String(x)).join(", ");
  return toEditString(v);
}

/**
 * A base field is an auto-generated identifier when its name ends in `_id`
 * (student_id, program_id, family_id, contact_id, tenant_id). These are
 * assigned by the system on data entry, so the model form must not invite a
 * value or a default for them.
 */
function isAutoGeneratedId(fieldName: string, isBase: boolean): boolean {
  return isBase && /_id$/.test(fieldName);
}
...
export default function FieldRow({
  ...
  const isAutoId = isAutoGeneratedId(fieldName, isBase);
    ...
    <>
      <tr className="field-row">
        <td className="field-row__name">
          {isBase ? (
            <span className="field-row__name-base">
              <code>{fieldName}</code>
              {isAutoId && (
                <span className="field-row__auto-chip" title="Auto-generated by the system on data entry">
                  auto
                </span>
              )}
            </span>
        ...
        <td className="field-row__value" data-testid="field-row-value">
          {isBase ? (
            <span
              className="field-row__display field-row__display--readonly"
              title={
                isAutoId
                  ? "Auto-generated on data entry — no value needed here"
                  : "Base field — populated when data is entered; no value needed in the model"
              }
            >
              {isAutoId ? (
                <em className="field-row__value-note">auto-generated</em>
              ) : (
                toReadonlyDisplay(value) || "—"
              )}
            </span>
        ...
        <td className="field-row__default" data-testid="field-row-default">
          {isAutoId ? (
            <span
              className="field-row__default-display field-row__default-display--readonly"
              title="Auto-generated — no default needed"
            >
              —
            </span>
          ) : fieldType === "selection" && (options?.length ?? 0) > 0 ? (
