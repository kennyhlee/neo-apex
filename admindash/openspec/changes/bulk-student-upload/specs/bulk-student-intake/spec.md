...
The system SHALL render all user-facing strings on the bulk-add page through the `useTranslation` hook, with translation keys defined for both `en-US` and `zh-CN` locales in `i18n/translations.ts`. New keys SHALL follow the existing nested-key convention used elsewhere in admindash.
