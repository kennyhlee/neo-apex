...
export default function AddLeadModal(
  ...
  // The model-driven branch renders DynamicForm, which owns its own
  // Cancel / Submit row, so the shared footer is only used by the
  // fixed-fields fallback.
  return (
    <Modal
      open
      onClose={onClose}
      ...
      footer={model ? undefined : (
        <>
          <Button variant="secondary" onClick={onClose} disabled={submitting}>Cancel</Button>
          <Button type="submit" form={formId} variant="primary" disabled={submitting}>Create</Button>
        </>
      )}
    >
      {model ? (
        ...
        <form id={formId} onSubmit={submitFixed}>
          {error && <div className="dynamic-form-error">{error}</div>}
