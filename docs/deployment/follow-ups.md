...
- **apexflow-api / familyhub-api bring-up remainder (hardening wave, 2026-08-08)** — repo artifacts are complete (`apexflow/fly.toml`+`Dockerfile`, `familyhub/fly.toml`+`Dockerfile`, `deploy.yml` jobs + tag regex, `suite-manifest.json` entries; commit `9e98354`). Both services now BOOT-REFUSE production misconfig (dev-default/short secrets, `TRUST_ALL_IPS=1` — live-demonstrated). Remaining operator steps: `fly apps create apexflow-api familyhub-api --org personal`; `fly secrets set` per each fly.toml header (generate 32+-char secrets; `FAMILYHUB_APEXFLOW_INTERNAL_KEY` must equal `APEXFLOW_INTERNAL_KEY`); `fly tokens create deploy -a <app>` → `gh secret set FLY_API_TOKEN_APEXFLOW` / `FLY_API_TOKEN_FAMILYHUB`; first `flyctl deploy`; Cloudflare DNS CNAMEs `api.apexflow.floatify.com` / `api.familyhub.floatify.com` (proxied) + `fly certs add`. Direct-to-Fly hits should 403 (allowlist); `/health` (apexflow) and `/api/health` (familyhub) exempt.

- **R2 provisioning for the documents seam** — R2 has never been enabled on the Cloudflare account (enabling is a card-on-file transaction; free tier 10GB). Needed before the document-upload legs of the Plan 3 checklist can run: enable R2 → create bucket (`neoapex-documents`) → API token (Object Read & Write) → export `DATACORE_R2_ENDPOINT` / `DATACORE_R2_ACCESS_KEY_ID` / `DATACORE_R2_SECRET_ACCESS_KEY` / `DATACORE_R2_BUCKET` in `~/.zshrc` (start-services.sh slurps `^export` lines) → restart DataCore (boto3 client is lru_cached). Then run plan `2026-08-06-hardening-wave.md` Tasks L1–L2. Also set the bucket-level 20 MB object cap while in the dashboard (see item above).
