# apexflow/backend/tests/test_cloudflare_ip.py
"""Tests for the Cloudflare IP allowlist middleware.

Copied (with app-specific health path already matching -- apexflow's health
route is `/health`, not `/api/health`) from familyhub/papermite/admindash/
launchpad's identical test file -- see app/middleware/cloudflare_ip.py's own
docstring for why this module is copy-pasted rather than shared.
"""
...
def test_real_cloudflare_to_fly_request_is_allowed(monkeypatch):
    """Regression test for the 'every parent is one rate-limit key' bug.
...
def test_health_endpoint_bypasses_ip_allowlist(monkeypatch):
    ...
    # /health must be reachable without any allowlist-passing header.
    monkeypatch.delenv("TRUST_ALL_IPS", raising=False)
    client = _make_app(trust_all_ips=False)
