# apexflow/backend/app/workflows/datacore.py
"""Structured DataCore client for the workflow engine.

Ported verbatim (module-relative import + docstring wording only) from
enrollx/backend/app/registration/datacore.py — see interface map §1. No
registration-specific logic lives in this module upstream either, so nothing
was removed.

DataCore's entity/query routes are unauthenticated by design (private-network
trust). `token` is forwarded when a staff JWT is present, and omitted on the
...
dc_create / dc_update / dc_query are BINDING names (interface map §1).

Injection hygiene: `tenant_id`, `entity_type`, and `entity_id` are always
id-shaped values (see `_ID_RE`) — they are validated with a strict allow-list
regex (mirroring admindash leads.py's `public_intake` check) before being
placed in a URL path or a SQL string, and any value this module itself
interpolates into SQL is additionally escaped with `sql_literal` (mirroring
admindash `chat/datacore.py`'s helper of the same name) as defense in depth.
The free-form `where` parameter accepted by `list_entities`/`get_entity` is
NOT validated or escaped by this module — see the trust-boundary note on
those functions.
"""
import json
import re
...
def get_model_definition(tenant_id: str, entity_type: str,
    ...
    rather than failing a caller's operation over a model that was never set
    up.
    """
    _validate_id(tenant_id, "tenant_id")
    _validate_id(entity_type, "entity_type")
