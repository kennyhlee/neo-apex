"""Query-guard dependencies for apexflow's /api/query proxy.

apexflow's per-{tenant_id}-path-param routes (app/api/entities.py) enforce
tenant scope through `app.auth.require_staff_tenant` already — see that
module's `# ADJUST(bindings)` note (auth.py binds admindash's
`require_tenant_match` name to enrollx's/apexflow's real
`require_staff_tenant`). This module exists only for the ONE route that has
no {tenant_id} path param: /api/query. DataCore scopes a query by the
